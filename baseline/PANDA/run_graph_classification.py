...
imdbm = list(TUDataset(root="data", name="IMDB-MULTI"))
nci1 = list(TUDataset(root="data", name="NCI1"))
dd = list(TUDataset(root="data", name="DD"))

datasets = {"reddit": reddit, "imdb": imdb, "mutag": mutag, "enzymes": enzymes, "proteins": proteins, "collab": collab,'imdbm':imdbm,'nci1':nci1,'dd':dd}

for key in datasets:
    if key in ["reddit", "imdb", "imdbm", "collab"]:
...
hyperparams = {
    ...
    "imdbm": AttrDict({"output_dim": 3}),
    "nci1": AttrDict({"output_dim": 2}),
    "reddit": AttrDict({"output_dim": 2}),
    "dd": AttrDict({"output_dim": 2})
}
...
for key in datasets:
    ...
    seed_set=[42,100,520,12345,1024]
    for trial in range(args.num_trials):
        init_seed(seed_set[trial])
        train_acc, validation_acc, test_acc, energy = Experiment(args=args, dataset=dataset,seed=seed_set[trial]).run()
